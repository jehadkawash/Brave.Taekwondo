...
const StudentPortal = ({ user, students, schedule, news, handleLogout }) => {
  ...
  // FIX #4: إصلاح اسم العائلة - entries كان يُطبع كـ Array
  const displayFamilyName = useMemo(() => {
    let name = currentUserData.familyName || 'عائلة';

    const isGeneric = 
      name.trim() === 'عائلة' || 
      name.trim().toLowerCase() === 'family';
    ...
    if (isGeneric) {
      ...
      // FIX #4: كان entries بدون [0][0] فيطبع Array كاملاً مثل "عائلة الخطيب,3"
      if (entries.length > 0) {
        name = `عائلة ${entries[0][0]}`;
      }
    }
      ...
      <header className="bg-slate-900/80 backdrop-blur-xl border-b border-slate-800 sticky top-0 z-40">
        <div className="container mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
             <div className="relative">
                ...
                <img 
                  src={IMAGES.LOGO} 
                  alt="Logo" 
                  className="w-10 h-10 md:w-12 md:h-12 relative z-10 bg-slate-900 rounded-full p-1 border border-yellow-500/20 object-contain" 
                />
             </div>
             <div>
                 {/* FIX #4: الآن displayFamilyName يعرض "عائلة الخطيب" وليس "عائلة الخطيب,3" */}
                 <h1 className="font-black text-lg md:text-xl text-slate-100 tracking-wide">
                   أهلاً <span className="text-yellow-500">{displayFamilyName}</span>
                 </h1>
          ...
          <div className="flex gap-2">
            <Button 
                onClick={() => {
                    const isHashed = currentUserData.isPasswordHashed || 
                      (currentUserData.password && currentUserData.password.length > 30);
                    setCreds({ 
            ...
            <Button 
              ...
              className="bg-red-900/20 hover:bg-red-900/30 text-red-400 border border-red-500/20 rounded-xl px-3 py-2 text-xs md:text-sm font-bold flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
                ...
                {relevantNews.length > 0 ? (
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    <div className="flex justify-between items-start mb-3">
                                        <span className="text-[10px] font-bold bg-yellow-500 text-slate-900 px-2 py-0.5 rounded-lg">
                                          {n.branch || 'عام'}
                                        </span>
                    ...
                    <div className="bg-slate-900 rounded-3xl p-8 flex items-center justify-center text-slate-600 h-[180px] border border-slate-800 border-dashed">
                        ...
                        <div className="h-28 bg-slate-950 border-b border-slate-800 p-6 flex justify-between items-center relative">
                            <div className="relative z-10 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <span className="text-xs font-bold text-slate-400 bg-slate-800 px-2 py-1 rounded-md border border-slate-700">
                                      {s.branch}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="p-6 md:p-8">
                            <div className="flex flex-wrap gap-4 mb-8">
                                ...
                                <div className="bg-slate-950 flex-1 min-w-[120px] rounded-2xl p-4 border border-slate-800">
                                    ...
                                    <p className={`text-xl font-black ${s.balance > 0 ? 'text-red-500' : 'text-emerald-500'}`}>
                                      {s.balance} JD
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-950/50 rounded-3xl p-5 border border-slate-800">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="font-bold text-slate-300 text-sm flex items-center gap-2">
                                      <Calendar size={16} className="text-blue-500"/> الحضور
                                    </h4>
                                    <div className="flex items-center gap-2 bg-slate-900 px-2 py-1 rounded-full border border-slate-800">
                                        <Button variant="ghost" onClick={()=>changeMonth(-1)} className="text-slate-500 p-0.5">
            ...
            <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-md">
                ...
                >
                    ...
                    <button 
                      ...
                      className="absolute top-4 left-4 text-slate-500 hover:text-red-500"
                    >
                      <X size={24}/>
                    </button>
                    ...
                    <form onSubmit={handleUpdateCredentials} className="p-8 space-y-6">
                        ...
                        <Button 
                          type="submit" 
                          disabled={isUpdating} 
                          className="w-full py-4 bg-yellow-500 text-slate-900 font-black hover:bg-yellow-400 rounded-xl shadow-lg shadow-yellow-500/20"
                        >
