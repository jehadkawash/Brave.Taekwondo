...
const StudentPortal = ({ user, students, schedule, news, handleLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] font-sans text-right text-white selection:bg-yellow-500 selection:text-black" dir="rtl">
        ...
        <div className="container mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
             <div className="relative">
                ...
                <img src={IMAGES.LOGO} alt="Logo" className="w-10 h-10 md:w-12 md:h-12 relative z-10 bg-black rounded-full p-1 border border-yellow-500/30 object-contain" />
             </div>
             <div>
                 <h1 className="font-black text-lg md:text-xl text-white tracking-wide">أهلاً <span className="text-yellow-500">{displayFamilyName}</span></h1>
                 <p className="text-[10px] md:text-xs text-gray-400 font-bold uppercase tracking-widest">بوابة العائلة</p>
             </div>
          </div>
          ...
          <div className="flex gap-2">
            <Button 
                ...
                className="bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-xl px-3 py-2 text-xs md:text-sm font-bold flex items-center gap-2 transition-all"
            >
                <Settings size={18}/> <span className="hidden md:inline">الإعدادات</span>
            </Button>
            <Button onClick={handleLogout} className="bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 rounded-xl px-3 py-2 text-xs md:text-sm font-bold flex items-center gap-2 transition-all">
      ...
      <div className="container mx-auto p-4 md:p-8 max-w-7xl space-y-10">
        ...
        {/* --- Top Dashboard Section (News + Payments) --- */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            
            {/* 1. News Slider (Takes 2/3 on large screens) */}
            <div className="lg:col-span-2 space-y-4">
                 <div className="flex items-center gap-2 mb-2">
                    <Megaphone size={20} className="text-yellow-500"/>
                    <h2 className="text-xl font-black text-white">آخر المستجدات</h2>
                </div>
                {relevantNews.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
                         {relevantNews.slice(0, 2).map((n, idx) => (
                            <motion.div 
                               ...
                               key={n.id} 
                               className="bg-white/5 border border-white/10 rounded-3xl p-5 hover:bg-white/10 transition-colors group relative overflow-hidden flex flex-col justify-between min-h-[160px]"
                            >
                                <div className="absolute top-0 right-0 w-20 h-20 bg-yellow-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 group-hover:bg-yellow-500/20 transition-colors"></div>
                                
                                <div className="relative z-10">
                                    <div className="flex justify-between items-start mb-3">
                                        <span className="text-[10px] font-bold bg-yellow-500 text-black px-2 py-0.5 rounded-lg">{n.branch || 'عام'}</span>
                                        <span className="text-[10px] text-gray-500 font-mono">{new Date(n.createdAt).toLocaleDateString('ar-EG')}</span>
                                    </div>
                                    <h3 className="font-bold text-lg text-white mb-2 line-clamp-2">{n.title}</h3>
                    ...
                    <div className="bg-white/5 rounded-3xl p-8 flex items-center justify-center text-gray-500 h-[180px]">لا يوجد أخبار حالياً</div>
                )}
            </div>

            {/* 2. Payments / Wallet (Takes 1/3 on large screens - MOVED TO TOP) */}
            <div className="lg:col-span-1">
                <div className="flex items-center gap-2 mb-4">
                    <Wallet size={20} className="text-green-500"/>
                    <h2 className="text-xl font-black text-white">المحفظة المالية</h2>
                </div>
                
                <div className="bg-gradient-to-br from-[#1a1a1a] to-black border border-white/10 rounded-[2rem] p-6 relative overflow-hidden h-full min-h-[220px]">
                    {/* Decorative Background */}
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-green-500 to-transparent"></div>
                    <div className="absolute bottom-0 right-0 w-32 h-32 bg-green-500/5 rounded-full blur-3xl"></div>

                    {myPayments.length > 0 ? (
                        <div className="relative z-10 h-full flex flex-col">
                            <p className="text-gray-400 text-xs font-bold mb-4">آخر 3 حركات مالية</p>
                            <div className="space-y-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                                {myPayments.slice(0, 3).map(p => (
                                    <div key={p.id} className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5 hover:border-green-500/30 transition-colors">
                                        <div className="w-8 h-8 rounded-full bg-green-500/10 flex items-center justify-center text-green-500 shrink-0">
                                            <ArrowDownLeft size={16}/>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-sm font-bold text-white truncate">{p.reason}</h4>
                                            <p className="text-[10px] text-gray-500 truncate">{p.name} • {p.date}</p>
                                        </div>
                                        <span className="text-green-400 font-bold font-mono text-sm">{p.amount} JD</span>
                                    </div>
                                ))}
                            </div>
                            {myPayments.length > 3 && (
                                <div className="mt-3 pt-3 border-t border-white/5 text-center">
                                    <span className="text-[10px] text-gray-500 cursor-pointer hover:text-white transition-colors">عرض كل السجل</span>
                                </div>
                            )}
                        </div>
                    ) : (
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3 border-b border-white/10 pb-4">
                ...
                <h2 className="text-2xl font-black text-white">الأبطال المسجلين</h2>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {myStudents.map((s, idx) => (
                    ...
                    >
                        {/* Student Card Header */}
                        <div className="h-28 bg-gradient-to-r from-gray-900 to-black border-b border-white/5 p-6 flex justify-between items-center relative overflow-hidden">
                            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10"></div>
                            <div className="relative z-10 flex items-center gap-4">
                                <div className={`w-3 h-12 rounded-full ${s.balance > 0 ? 'bg-red-500' : 'bg-green-500'} shadow-[0_0_15px_rgba(0,0,0,0.5)]`}></div>
                                <div>
                                    <h3 className="text-2xl font-black text-white mb-1">{s.name}</h3>
                                    <span className="text-xs font-bold text-gray-400 bg-white/5 px-2 py-1 rounded-md border border-white/5">{s.branch}</span>
                                </div>
                            </div>
                            <div className="relative z-10 text-center">
                                <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">الحزام</p>
                        ...
                        <div className="p-6 md:p-8">
                            ...
                            <div className="flex flex-wrap gap-4 mb-8">
                                <div className="bg-white/5 flex-1 min-w-[120px] rounded-2xl p-4 border border-white/5">
                                    <p className="text-[10px] text-gray-500 font-bold mb-2">الاشتراك</p>
                                    <div className="flex items-center gap-2 mb-1">
                                        <StatusBadge status={calculateStatus(s.subEnd)}/>
                                    </div>
                                    <p className="text-[10px] text-gray-400 font-mono">{s.subEnd}</p>
                                </div>
                                <div className="bg-white/5 flex-1 min-w-[120px] rounded-2xl p-4 border border-white/5">
                                    <p className="text-[10px] text-gray-500 font-bold mb-2">الرصيد</p>
                                    <p className={`text-xl font-black ${s.balance > 0 ? 'text-red-500' : 'text-green-500'}`}>{s.balance} JD</p>
                                    <p className="text-[10px] text-gray-400 mt-1">{s.balance > 0 ? 'مستحق للدفع' : 'مدفوع'}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-black/40 rounded-3xl p-5 border border-white/5">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="font-bold text-white text-sm flex items-center gap-2"><Calendar size={16} className="text-blue-500"/> الحضور</h4>
                                    <div className="flex items-center gap-2 bg-white/5 px-2 py-1 rounded-full border border-white/5">
                                        <Button variant="ghost" onClick={()=>changeMonth(-1)} className="text-gray-400 hover:text-white p-0.5"><ChevronRight size={14}/></Button>
                                        <span className="text-xs font-bold font-mono dir-ltr">{monthNames[month]} {year}</span>
                                        <Button variant="ghost" onClick={()=>changeMonth(1)} className="text-gray-400 hover:text-white p-0.5"><ChevronLeft size={14}/></Button>
                                    </div>
                                </div>
                                    ...
                                    {[...Array(daysInMonth)].map((_,i)=>{
                                        ...
                                        return (
                                            <div key={d} className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold transition-all
                                                ${isPresent ? 'bg-green-500 text-black shadow-lg shadow-green-500/20' : 'bg-white/5 text-gray-600'}`}>
                                                {d}
                                            </div>
                                        )
                                    })}
        ...
        <div className="space-y-6 pb-10">
            <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-black text-white">جدول الحصص</h2>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
                        {schedule.map(cls => (
                            <div key={cls.id} className="bg-white/5 p-5 rounded-2xl border border-white/5 hover:border-yellow-500/30 transition-colors group">
                                <div className="flex justify-between items-start mb-3">
                                    <span className="text-[10px] font-bold text-gray-400 bg-black/40 px-2 py-1 rounded-md">{cls.branch}</span>
                                    <Clock size={16} className="text-gray-600 group-hover:text-yellow-500 transition-colors"/>
                                </div>
                                <h4 className="text-lg font-black text-white mb-2">{cls.level}</h4>
