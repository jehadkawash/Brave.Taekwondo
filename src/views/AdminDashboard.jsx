...
const AdminDashboard = ({ user, selectedBranch, studentsCollection, scheduleCollection, handleLogout, onSwitchBranch }) => {
  ...
  const handleBackup = () => {
    ...
    const backupData = { date: new Date().toISOString(), branch: selectedBranch, students: branchStudents, payments: branchPayments, expenses: branchExpenses, registrations: branchRegistrations, schedule: schedule, news: newsData, groups: branchGroups, captains: captains, activityLogs: branchActivityLogs, adminNotes: branchAdminNotes };
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(backupData, null, 2));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `brave_backup_${selectedBranch}_${new Date().toISOString().split('T')}.json`);
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
  };

  // --- بناء القوائم بناءً على الصلاحيات ---
  const studentGroups = [
      hasPerm('students') && {id:'students', icon:Users, label:'الطلاب'}, 
      hasPerm('student_notes') && {id:'student_notes', icon:MessageSquare, label:'الملاحظات والرسائل'}, 
  ...
  const adminGroups = [
      ...
      hasPerm('notes') && {id:'notes', label: 'ملاحظات الإدارة', icon: FileText },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex flex-col selection:bg-yellow-500/30" dir="rtl">
              ...
              <nav className="hidden md:flex items-center gap-2">
                  {hasPerm('dashboard') && (
                      <button 
                          ...
                          className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all duration-300 font-bold text-sm border border-transparent ${activeTab === 'dashboard' ? 'text-yellow-400 bg-slate-800 border-slate-700 shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                      >
                  ...
                  {studentGroups.length > 0 && <NavDropdown title="طلاب" icon={Users} items={studentGroups} activeTab={activeTab} onSelect={setActiveTab} />}

                  {hasPerm('attendance') && (
                      <button 
                          onClick={() => setActiveTab('attendance')}
                          className={`flex items-center gap-2 px-4 py-2 mx-2 rounded-xl transition-all duration-300 font-bold text-sm border ${activeTab === 'attendance' ? 'bg-yellow-500 text-slate-900 border-yellow-400 shadow-[0_0_15px_rgba(234,179,8,0.4)] scale-105' : 'border-yellow-500/50 text-yellow-500 hover:bg-yellow-500 hover:text-slate-900'}`}
                      >
                          <CheckCircle size={20}/> حضور وغياب
                      </button>
                  )}

                  {adminGroups.length > 0 && <NavDropdown title="الادارة" icon={Shield} items={adminGroups} activeTab={activeTab} onSelect={setActiveTab} />}
              </nav>
              ...
              <div className="flex items-center gap-3">
                  {onSwitchBranch && (
                      <div className="hidden md:block relative group">
                          ...
                          <select value={selectedBranch} onChange={(e) => onSwitchBranch(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer">
              ...
              <div className="md:hidden bg-slate-900/95 backdrop-blur-xl border-t border-slate-800 p-4 space-y-4 animate-in slide-in-from-top-5 duration-300 absolute w-full left-0 z-50 shadow-2xl h-[calc(100vh-64px)] overflow-y-auto">
                  ...
                  {onSwitchBranch && (
                      <div className="bg-slate-800/50 border border-slate-700 p-3 rounded-xl flex items-center justify-between mb-4">
                          ...
                          <select value={selectedBranch} onChange={(e) => onSwitchBranch(e.target.value)} className="bg-slate-950 text-yellow-500 font-bold p-2 px-4 rounded-lg outline-none text-sm border border-slate-700">
                              <option value="شفا بدران">شفا بدران</option>
                              <option value="أبو نصير">أبو نصير</option>
                          </select>
                      </div>
                  ...
                  {hasPerm('dashboard') && (
                      <button onClick={() => {setActiveTab('dashboard'); setMobileMenuOpen(false);}} className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-800 text-white font-bold border border-slate-700">
                          <Activity size={20} className="text-yellow-500"/> الرئيسية
                      </button>
                  )}

                  {hasPerm('attendance') && (
                      <button onClick={() => {setActiveTab('attendance'); setMobileMenuOpen(false);}} className="w-full flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-yellow-600 to-yellow-500 text-slate-900 font-bold">
                          <CheckCircle size={20}/> حضور وغياب
                      </button>
                  )}
                      ...
                      <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                              {studentGroups.map(item => (
                                  <button key={item.id} onClick={() => {setActiveTab(item.id); setMobileMenuOpen(false);}} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl text-xs font-bold border transition-all duration-200 ${activeTab === item.id ? 'bg-yellow-500 text-slate-900 border-yellow-500 shadow-md' : 'bg-slate-800/50 text-slate-400 border-slate-800'}`}>
                      ...
                      <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                              {adminGroups.map(item => (
                                  <button key={item.id} onClick={() => { if (item.action) item.action(); else setActiveTab(item.id); setMobileMenuOpen(false); }} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl text-xs font-bold border transition-all duration-200 ${item.special ? 'text-emerald-400 border-emerald-900/50 bg-emerald-900/10' : activeTab === item.id ? 'bg-yellow-500 text-slate-900 border-yellow-500' : 'bg-slate-800/50 text-slate-400 border-slate-800'}`}>
                                      <item.icon size={22}/> {item.label}
                                  </button>
                              ))}
                          </div>
      ...
      <main className="flex-1 w-full px-4 md:px-8 py-8 animate-in fade-in slide-in-from-bottom-4 duration-500 mx-auto">
         <div className="bg-slate-900/0 rounded-2xl min-h-[500px]">
             {activeTab === 'dashboard' && hasPerm('dashboard') && <DashboardStats user={user} selectedBranch={selectedBranch} branchStudents={branchStudents} netProfit={netProfit} totalAttendance={totalAttendance} expiredCount={expiredCount} activeStudentsCount={activeStudentsCount} nearEndCount={nearEndCount} totalStudents={totalStudents} branchRegistrations={branchRegistrations} branchPayments={branchPayments} activityLogs={branchActivityLogs}/>}
             {activeTab === 'students' && hasPerm('students') && <StudentsManager students={branchStudents} groups={branchGroups} studentsCollection={studentsCollection} archiveCollection={archiveCollection} selectedBranch={selectedBranch} logActivity={handleLog}/>}
             {activeTab === 'tests' && hasPerm('tests') && <BeltTestsManager students={branchStudents} studentsCollection={studentsCollection} logActivity={handleLog}/>}
             {activeTab === 'reports' && hasPerm('reports') && <ReportsManager students={branchStudents} payments={branchPayments} expenses={branchExpenses} activityLogs={branchActivityLogs} registrations={branchRegistrations} adminNotes={branchAdminNotes} selectedBranch={selectedBranch}/>}
             {activeTab === 'subscriptions' && hasPerm('subscriptions') && <SubscriptionsManager students={branchStudents} studentsCollection={studentsCollection} logActivity={handleLog} selectedBranch={selectedBranch}/>}
             {activeTab === 'finance' && hasPerm('finance') && <FinanceManager students={branchStudents} payments={branchPayments} expenses={branchExpenses} paymentsCollection={paymentsCollection} expensesCollection={expensesCollection} financeReasons={branchFinanceReasons} financeReasonsCollection={financeReasonsCollection} selectedBranch={selectedBranch} logActivity={handleLog}/>}
             {activeTab === 'attendance' && hasPerm('attendance') && <AttendanceManager students={branchStudents} groups={branchGroups} groupsCollection={groupsCollection} studentsCollection={studentsCollection} selectedBranch={selectedBranch}/>}
             {activeTab === 'registrations' && hasPerm('registrations') && <RegistrationsManager registrations={branchRegistrations} students={students} registrationsCollection={registrationsCollection} studentsCollection={studentsCollection} selectedBranch={selectedBranch} logActivity={handleLog}/>}
             {activeTab === 'schedule' && hasPerm('schedule') && <ScheduleManager schedule={schedule} scheduleCollection={scheduleCollection}/>}
             {activeTab === 'archive' && hasPerm('archive') && <ArchiveManager archiveCollection={archiveCollection} studentsCollection={studentsCollection} payments={payments} logActivity={handleLog}/>}
             {activeTab === 'events' && hasPerm('events') && <EventsManager students={branchStudents} logActivity={handleLog}/>}
             {activeTab === 'weight' && hasPerm('weight') && <WeightTracker students={branchStudents} logActivity={handleLog}/>}
             {activeTab === 'captains' && user.isSuper && <CaptainsManager captains={captains} captainsCollection={captainsCollection}/>}
             {activeTab === 'news' && hasPerm('news') && <NewsManager news={newsData} newsCollection={newsCollection} selectedBranch={selectedBranch}/>}
             {activeTab === 'notes' && hasPerm('notes') && <AdminNotesManager/>}
             {activeTab === 'student_notes' && hasPerm('student_notes') && <NotesManager students={branchStudents} studentsCollection={studentsCollection} logActivity={handleLog} selectedBranch={selectedBranch}/>}
         </div>
      </main>
    </div>
  );
