...
// ... (نفس الاستيرادات القديمة، تأكد من إضافة StoreManager و ReceiptsManager)
import { Activity, Users, DollarSign, CheckCircle, Inbox, Clock, Archive, Shield, Menu, LogOut, Megaphone, Download, Database, ShoppingBag, FileText } from 'lucide-react';
...
import FinanceManager from './dashboard/FinanceManager'; // (السوبر أدمن فقط)
import ReceiptsManager from './dashboard/ReceiptsManager'; // (الجديد - للكباتن)
import StoreManager from './dashboard/StoreManager'; // (الجديد - للسوبر أدمن)
...
// ... (دالة calculateStatus ودالة logActivity كما هي) ...
...
const AdminDashboard = ({ user, selectedBranch, studentsCollection, scheduleCollection, handleLogout }) => {
  ...
  // ... (باقي الكولكشنز الأخبار والجروبات واللوج كما هي) ...
  ...
  const navItems = [
    ...
    {id:'receipts',icon:FileText,label:'الوصولات والمبيعات'}, // للجميع (كابتن + أدمن)
    {id:'students',icon:Users,label:'الطلاب'},
    // {id:'finance',icon:DollarSign,label:'المالية'}, ❌ حذفنا القديم
    {id:'attendance',icon:CheckCircle,label:'الحضور'},
    {id:'schedule',icon:Clock,label:'الجدول'},
    
    // 👇 أقسام السوبر أدمن فقط
    {id:'store',icon:ShoppingBag,label:'إدارة المتجر', role: 'admin'},
    {id:'finance_reports',icon:DollarSign,label:'التقارير المالية', role: 'admin'},
    {id:'archive',icon:Archive,label:'الأرشيف', role: 'admin'},
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 text-right font-sans" dir="rtl">
      ...
      <main className="flex-1 p-4 md:p-8 w-full overflow-x-hidden">
         ...
         {/* ✅ صفحة إدارة المتجر (أدمن فقط) */}
         {activeTab === 'store' && user.role === 'admin' && <StoreManager 
             productsCollection={productsCollection}
             selectedBranch={selectedBranch}
         />}

         {/* ✅ صفحة التقارير المالية (أدمن فقط) */}
         {activeTab === 'finance_reports' && user.role === 'admin' && <FinanceManager 
             selectedBranch={selectedBranch}
             logActivity={handleLog}
         />}
