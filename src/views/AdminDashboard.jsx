  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      >
        ...
        <ChevronDown
          size={14}
          className={`transition-transform duration-300 ${isOpen ? 'rotate-180 text-yellow-500' : ''}`}
        />
      </button>

      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-64 bg-slate-900 rounded-xl shadow-2xl shadow-black border border-slate-700 overflow-hidden z-50">
          {items.map((item, idx) => (
            <button
              key={idx}
              onClick={() => {
            ...
            >
              <item.icon
                ...
                className={`transition-colors duration-200 ${
                  activeTab === item.id ? 'text-yellow-400'
                  : item.special ? 'text-emerald-400'
                  : 'text-slate-500 group-hover:text-slate-300'}`}
              />
              {item.label}
              {item.badge > 0 && (
                <span className="mr-auto bg-red-600 text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm shadow-red-900/50">
                  {item.badge}
                </span>
              )}
            </button>
...
const AdminDashboard = ({
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex flex-col selection:bg-yellow-500/30" dir="rtl">
      ...
      <header className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800 text-white shadow-lg sticky top-0 z-40 transition-all duration-300">
        <div className="container mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 group cursor-default">
            <div className="relative">
              <img
                src={IMAGES.LOGO}
                alt="Logo"
                className="w-10 h-10 rounded-xl object-contain bg-white shadow-lg shadow-yellow-500/20"
          ...
          <nav className="hidden md:flex items-center gap-2">
            {hasPerm('dashboard') && (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all duration-300 font-bold text-sm border border-transparent
                  ${activeTab === 'dashboard'
                    ? 'text-yellow-400 bg-slate-800 border-slate-700 shadow-lg'
                    : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
              >
            ...
            {studentGroups.length > 0 && (
              <NavDropdown title="طلاب" icon={Users} items={studentGroups} activeTab={activeTab} onSelect={setActiveTab} />
            )}
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 mx-2 rounded-xl transition-all duration-300 font-bold text-sm border
                  ${activeTab === 'attendance'
                    ...
                    : 'border-yellow-500/50 text-yellow-500 hover:bg-yellow-500 hover:text-slate-900'}`}
              >
            ...
            {adminGroups.length > 0 && (
              <NavDropdown title="الادارة" icon={Shield} items={adminGroups} activeTab={activeTab} onSelect={setActiveTab} />
            )}
          </nav>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              className="md:hidden p-2 text-slate-200 hover:bg-slate-800 rounded-lg"
        ...
        {/* Mobile menu */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-slate-900/95 backdrop-blur-xl border-t border-slate-800 p-4 space-y-4 absolute w-full left-0 z-50 shadow-2xl h-[calc(100vh-64px)] overflow-y-auto">
            {onSwitchBranch && (
              <div className="bg-slate-800/50 border border-slate-700 p-3 rounded-xl flex items-center justify-between mb-4">
                ...
                <select
                  value={selectedBranch}
                  onChange={(e) => onSwitchBranch(e.target.value)}
                  className="bg-slate-950 text-yellow-500 font-bold p-2 px-4 rounded-lg outline-none text-sm border border-slate-700"
                >
                  <option value="شفا بدران">شفا بدران</option>
                  <option value="أبو نصير">أبو نصير</option>
                </select>
            ...
            {hasPerm('dashboard') && (
              <button
                ...
                className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-800 text-white font-bold border border-slate-700"
              >
            ...
            {hasPerm('attendance') && (
              <button
                ...
                className="w-full flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-yellow-600 to-yellow-500 text-slate-900 font-bold"
              >
                <CheckCircle size={20} /> حضور وغياب
              </button>
            )}
      ...
      {/* ── Main ── */}
      <main className="flex-1 w-full px-4 md:px-8 py-8 mx-auto">
        <div className="bg-slate-900/0 rounded-2xl min-h-[500px]">
          ...
          {activeTab === 'subscriptions'  && hasPerm('subscriptions')  && (
          ...
          {activeTab === 'attendance'     && hasPerm('attendance')     && (
            <AttendanceManager
              students={branchStudents}
              groups={branchGroups}
          ...
          {activeTab === 'registrations'  && hasPerm('registrations')  && (
