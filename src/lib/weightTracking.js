...
export function scopedWeightGoal(rows,id,eventId=null){return rows.filter(r=>r.studentId===id&&r._isTarget&&(r.eventId||null)===eventId).sort((a,b)=>(weightDateMillis(b.updatedAt||b.createdAt)||0)-(weightDateMillis(a.updatedAt||a.createdAt)||0)||String(a.id).localeCompare(String(b.id)))[0]||null;}
export function studentWeightData(rows,id){const all=rows.filter(r=>r.studentId===id);return {entries:all.filter(r=>!r._isTarget&&!r._voided).sort(compareWeightEntries),goal:scopedWeightGoal(rows,id)};}
export function previewWeight(entries,weight,when,goal){const time=weightDateMillis(when);const previous=entries.find(r=>weightDateMillis(measurementDate(r))!==null&&weightDateMillis(measurementDate(r))<=time);return {change:changeLabel({weight},previous),target:goalStatus(weight,goal).text};}
